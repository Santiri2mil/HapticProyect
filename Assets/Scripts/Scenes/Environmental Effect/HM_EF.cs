...
public class HM_EF : MonoBehaviour
{
    // plugin import
    ...
    private Vector3[] tempDesiredPos = new Vector3[16];
    ...
    void Start()
    {
        ...
        if (hapticDevices > 0)
        {
            ...
            {
                myHIP[i] = (HIP_EF)hapticCursors[i].GetComponent(typeof(HIP_EF));
    ...
    void HapticThread()
    {
        ...
        while (hapticThreadIsRunning)
        {
            for (int i = 0; i < hapticDevices; i++)
            {
                ...
                // get desired position from hip and convert it into haptic device positions
                ...
                if (myHIP[i].enableForce)
                {
                    SetForceByDesiredPosition(i, tempDesiredPos[i]);
                }
